...
var topic = getenv("TOPIC", "topic30")
var brokers = getenv("BROKERS", "localhost:9093")
...
func async() {
	...
	fmt.Printf("Connecting to BROKERS=%s TOPIC=%s\n", brokers, topic)
	config, err := kafkaConfig()
	if err != nil {
		fmt.Println(err.Error())
	}
	//fmt.Printf("Using with config %v\n", config)

	producer, err := sarama.NewAsyncProducer([]string{brokers}, config)
	if err != nil {
		fmt.Println(err)
		panic("Error creating the async producer")
	}
	i := 11
